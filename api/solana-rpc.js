...
export default async function handler(request) {
  ...
  try {
    ...
    return json({ error: { code: -32700, message: "Invalid JSON request." } }, 400);
  }
  ...
  try {
    ...
    try {
      JSON.parse(body); // upstream said 200 but may still send HTML/text under load
    } catch {
      return json(
        { error: { code: -32000, message: "The configured Solana RPC returned a non-JSON response." } },
        502
      );
    }
