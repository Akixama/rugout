...
export default function Sniffer({ pose = "idle", size = 96 }) {
  ...
  // Idle blink loop — only while eyes are the plain dot style.
  useEffect(() => {
    if (pose !== "idle" && pose !== "frozen") return undefined;
    let timeoutId;
    const scheduleBlink = () => {
  ...
  // Pop reaction whenever the pose actually changes.
  useEffect(() => {
  ...
  return (
    <svg
      ...
      className={`sniffer sniffer-${pose}`}
    >
      {/* rug corner */}
      <path
      ...
      <g className="sniffer-body">
        ...
        {(pose === "idle" || pose === "frozen") && (
          <g>
            {blink ? (
              <>
                <path d="M45 67 Q50 71 55 67" stroke={outline} strokeWidth="3" fill="none" strokeLinecap="round" />
                <path d="M65 67 Q70 71 75 67" stroke={outline} strokeWidth="3" fill="none" strokeLinecap="round" />
              </>
            ) : (
              <>
